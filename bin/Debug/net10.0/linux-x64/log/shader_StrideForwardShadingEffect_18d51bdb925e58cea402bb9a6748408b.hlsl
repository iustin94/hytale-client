...
@R    LinearSampler_id147 => Texturing.LinearSampler [Stage: Pixel, Slot: (-1--1)]
@R    EnvironmentLightingDFG_LUT_id175 => MaterialSpecularMicrofacetEnvironmentGGXLUT.EnvironmentLightingDFG_LUT [Stage: Pixel, Slot: (-1--1)]
...
*****     Stages      *****
***************************
@G    Vertex => ff877ae387f722f39b140a37287ad810
@G    Pixel => a650af3158e929e2dfd9dcd8acb469fe
***************************
*************************/
...
float VisibilityhSchlickGGX_id147(float alphaR, float nDotX)
{
    float k = alphaR * 0.5f;
    return nDotX / (nDotX * (1.0f - k) + k);
}
float3 FresnelSchlick_id131(float3 f0, float3 f90, float lOrVDotH)
{
...
void ProcessLight_id85(inout PS_STREAMS streams, SpotLightDataInternal light)
{
    ...
    float attenuation = ComputeAttenuation_id84(light.PositionWS, light.AngleOffsetAndInvSquareRadius, light.DirectionWS, streams.PositionWS_id23.xyz, lightVectorNorm);
...
{
    float3 lightVectorNorm;
    float attenuation = ComputeAttenuation_id71(light, streams.PositionWS_id23.xyz, lightVectorNorm);
    streams.lightPositionWS_id42 = light.PositionWS;
    streams.lightColor_id44 = light.Color;
    streams.lightAttenuation_id47 = attenuation;
    streams.lightDirectionWS_id43 = lightVectorNorm;
}
float NormalDistributionGGX_id161(float alphaR, float nDotH)
{
    float alphaR2 = alphaR * alphaR;
    float d = max(nDotH * nDotH * (alphaR2 - 1) + 1, 0.0001);
    return alphaR2 / (PI_id116 * d * d);
}
float VisibilitySmithSchlickGGX_id149(float alphaR, float nDotL, float nDotV)
{
    return VisibilityhSchlickGGX_id147(alphaR, nDotL) * VisibilityhSchlickGGX_id147(alphaR, nDotV) / (nDotL * nDotV);
}
float3 FresnelSchlick_id137(float3 f0, float lOrVDotH)
{
    return FresnelSchlick_id131(f0, 1.0f, lOrVDotH);
}
float3 Compute_id214(float3 specularColor, float alphaR, float nDotV)
{
    float glossiness = 1.0f - sqrt(alphaR);
    float4 environmentLightingDFG = EnvironmentLightingDFG_LUT_id175.SampleLevel(LinearSampler_id147, float2(glossiness, nDotV), 0);
...
{
    return NormalDistributionGGX_id161(streams.alphaRoughness_id71, streams.NdotH_id77);
}
float Compute_id278(inout PS_STREAMS streams)
{
    return VisibilitySmithSchlickGGX_id149(streams.alphaRoughness_id71, streams.NdotL_id50, streams.NdotV_id73);
}
float3 Compute_id247(inout PS_STREAMS streams, float3 f0)
{
    return FresnelSchlick_id137(f0, streams.LdotH_id78);
}
...
void ResetStream_id164()
{
}
void AfterLightingAndShading_id368()
{
}
void AfterLightingAndShading_id348()
{
}
float3 ComputeEnvironmentLightContribution_id399(inout PS_STREAMS streams)
{
    ...
    return Compute_id214(specularColor, streams.alphaRoughness_id71, streams.NdotV_id73) * streams.envLightSpecularColor_id49;
}
float3 ComputeEnvironmentLightContribution_id341(inout PS_STREAMS streams)
{
...
void PrepareDirectLight_id198(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.shadowColor_id84 = ComputeShadow_id78(streams, streams.PositionWS_id23.xyz, lightIndex);
    ...
    streams.lightColorNdotL_id45 *= ComputeTextureProjection_id79(streams.PositionWS_id23.xyz, lightIndex);
    float3 reflectionVectorWS = reflect(-streams.viewWS_id69, streams.normalWS_id20);
    streams.lightSpecularColorNdotL_id46 *= ComputeSpecularTextureProjection_id80(streams.PositionWS_id23.xyz, reflectionVectorWS, lightIndex);
}
...
void PrepareDirectLight_id185(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.NdotL_id50 = max(dot(streams.normalWS_id20, streams.lightDirectionWS_id43), 0.0001f);
    streams.shadowColor_id84 = ComputeShadow_id67(streams, streams.PositionWS_id23.xyz, lightIndex);
    streams.lightColorNdotL_id45 = streams.lightColor_id44 * streams.lightAttenuation_id47 * streams.shadowColor_id84 * streams.NdotL_id50 * streams.lightDirectAmbientOcclusion_id51;
    streams.lightSpecularColorNdotL_id46 = streams.lightColorNdotL_id45;
    streams.lightColorNdotL_id45 *= ComputeTextureProjection_id68(streams.PositionWS_id23.xyz, lightIndex);
    float3 reflectionVectorWS = reflect(-streams.viewWS_id69, streams.normalWS_id20);
    streams.lightSpecularColorNdotL_id46 *= ComputeSpecularTextureProjection_id69(streams.PositionWS_id23.xyz, reflectionVectorWS, lightIndex);
}
...
float3 ComputeDirectLightContribution_id398(inout PS_STREAMS streams)
{
    ...
    float geometricShadowing = Compute_id278(streams);
...
float3 ComputeDirectLightContribution_id340(inout PS_STREAMS streams)
{
    float3 diffuseColor = streams.matDiffuseVisible_id70;
    return diffuseColor / PI_id116 * streams.lightColorNdotL_id45 * streams.matDiffuseSpecularAlphaBlend_id66.x;
}
...
void PrepareDirectLight_id174(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.NdotL_id50 = max(dot(streams.normalWS_id20, streams.lightDirectionWS_id43), 0.0001f);
    streams.shadowColor_id84 = ComputeShadow_id61(streams, streams.PositionWS_id23.xyz, lightIndex);
    streams.lightColorNdotL_id45 = streams.lightColor_id44 * streams.lightAttenuation_id47 * streams.shadowColor_id84 * streams.NdotL_id50 * streams.lightDirectAmbientOcclusion_id51;
    streams.lightSpecularColorNdotL_id46 = streams.lightColorNdotL_id45;
...
int GetLightCount_id176()
{
    return LightCount_id86;
}
int GetMaxLightCount_id177()
{
...
void PrepareForLightingAndShading_id365()
{
}
void PrepareForLightingAndShading_id345()
{
}
void PrepareMaterialForLightingAndShading_id163(inout PS_STREAMS streams)
{
...
void ResetLightStream_id162(inout PS_STREAMS streams)
{
...
void Compute_id431(inout PS_STREAMS streams)
{
    ...
    ResetLightStream_id162(streams);
    PrepareMaterialForLightingAndShading_id163(streams);

    {
        PrepareForLightingAndShading_id345();
    }

    {
        PrepareForLightingAndShading_id365();
    }
    float3 directLightingContribution = 0;

    {
        PrepareDirectLights_id172();
        const int maxLightCount = GetMaxLightCount_id177();
        int count = GetLightCount_id176();
        ...
        for (int i = 0; i < maxLightCount; i++)
        {
            ...
            {
                directLightingContribution += ComputeDirectLightContribution_id340(streams);
            }
...

    {
        ...
        {
            environmentLightingContribution += ComputeEnvironmentLightContribution_id341(streams);
        }
    ...
    streams.shadingColor_id74 += directLightingContribution * PI_id116 + environmentLightingContribution;
    streams.shadingColorAlpha_id75 = streams.matDiffuse_id54.a;

    {
        AfterLightingAndShading_id348();
    }

    {
        AfterLightingAndShading_id368();
    }
}
void Compute_id414(inout PS_STREAMS streams)
{
    float metalness = Compute_id212().r;
...
{
    float glossiness = Compute_id210().r;
...
float3 EncodeNormal_id54(float3 n)
{
    return n * 0.5 + 0.5;
}
...
float4 Compute_id47(inout PS_STREAMS streams)
{
    return float4(EncodeNormal_id54(streams.normalWS_id20), 1);
}
...
void PSMain_id3(inout PS_STREAMS streams)
{
    ...
    streams.ColorTarget1_id3 = Compute_id47(streams);
    streams.ColorTarget2_id4 = Compute_id48(streams);
}
