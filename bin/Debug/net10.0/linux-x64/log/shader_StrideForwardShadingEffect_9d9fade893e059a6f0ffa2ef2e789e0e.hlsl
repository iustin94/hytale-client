...
cbuffer PerMaterial [Size: 24]
@C    constantColor_id186 => Material.DiffuseValue
@C    constantFloat_id188 => Material.GlossinessValue
@C    constantFloat_id191 => Material.MetalnessValue
cbuffer PerView [Size: 1712]
...
@R    LinearSampler_id125 => Texturing.LinearSampler [Stage: Pixel, Slot: (-1--1)]
@R    LinearClampCompareLessEqualSampler_id127 => Texturing.LinearClampCompareLessEqualSampler [Stage: Pixel, Slot: (-1--1)]
@R    EnvironmentLightingDFG_LUT_id198 => MaterialSpecularMicrofacetEnvironmentGGXLUT.EnvironmentLightingDFG_LUT [Stage: Pixel, Slot: (-1--1)]
@R    EnvironmentLightingDFG_LUT_id198 => MaterialSpecularMicrofacetEnvironmentGGXLUT.EnvironmentLightingDFG_LUT [Stage: None, Slot: (-1--1)]
...
*****     Stages      *****
***************************
@G    Vertex => 5a7df1ce4562caf439e326b7715e7a7d
@G    Pixel => f8344ae03a87f74774e00a54497dc18c
***************************
*************************/
...
const static bool TInvert_id189 = false;
...
Texture2D EnvironmentLightingDFG_LUT_id198;
...
float4 Project_id73(float4 vec, float4x4 mat)
{
...
float Get3x3FilterKernel_id83(float2 base_uv, float2 st, out float3 kernel[4])
{
    float2 uvW0 = (3 - 2 * st);
    float2 uvW1 = (1 + 2 * st);
...
float Get5x5FilterKernel_id82(float2 base_uv, float2 st, out float3 kernel[9])
{
    float2 uvW0 = (4 - 3 * st);
    float2 uvW1 = 7;
    float2 uvW2 = (1 + 3 * st);
...
float SampleThickness_id84(float3 shadowSpaceCoordinate, float3 pixelPositionWS, float2 depthRanges, float4x4 inverseWorldToShadowCascadeUV, bool isOrthographic)
{
    ...
    {
        float4 shadowmapPositionWorldSpace = Project_id73(float4(shadowSpaceCoordinate.xy, shadowMapDepth, 1.0), inverseWorldToShadowCascadeUV);
        thickness = distance(shadowmapPositionWorldSpace.xyz, pixelPositionWS.xyz);
    }
    return (thickness);
}
float Get7x7FilterKernel_id80(float2 base_uv, float2 st, out float3 kernel[16])
{
    float2 uvW0 = (5 * st - 6);
...
float GetAngularAttenuation_id108(float3 lightVector, float3 lightDirection, float lightAngleScale, float lightAngleOffset)
{
...
float SampleAndFilter_id88(float3 adjustedPixelPositionWS, float3 adjustedPixelPositionShadowSpace, float2 depthRanges, float4x4 inverseWorldToShadowCascadeUV, bool isOrthographic, bool isDualParaboloid = false)
{
    ...
    {
        float3 kernel[9];
        normalizationFactor = Get5x5FilterKernel_id82(base_uv, st, kernel);

        [unroll]
        for (int i = 0; i < 9; ++i)
        {
            thickness += kernel[i].z * SampleThickness_id84(float3(kernel[i].xy, adjustedPixelPositionShadowSpace.z), adjustedPixelPositionWS, depthRanges, inverseWorldToShadowCascadeUV, isOrthographic);
        }
    }
    return (thickness / normalizationFactor);
}
void CalculateAdjustedShadowSpacePixelPosition_id87(float filterRadiusInPixels, float3 pixelPositionWS, float3 meshNormalWS, float4x4 worldToShadowCascadeUV, float4x4 inverseWorldToShadowCascadeUV, out float3 adjustedPixelPositionWS, out float3 adjustedPixelPositionShadowSpace)
{
    float4 bottomLeftTexelWS = Project_id73(float4(0.0, 0.0, 0.0, 1.0), inverseWorldToShadowCascadeUV);
    const float4 topRightTexelWS = Project_id73(float4(ShadowMapTextureTexelSize_id94.xy * filterRadiusInPixels, 0.0, 1.0), inverseWorldToShadowCascadeUV);
    const float texelDiagonalLength = distance(topRightTexelWS.xyz, bottomLeftTexelWS.xyz);
    const float3 positionOffsetWS = meshNormalWS * texelDiagonalLength;
    adjustedPixelPositionWS = pixelPositionWS - positionOffsetWS;
    const float4 shadowMapCoordinate = Project_id73(float4(adjustedPixelPositionWS, 1.0), worldToShadowCascadeUV);
    adjustedPixelPositionShadowSpace = shadowMapCoordinate.xyz;
}
void CalculateAdjustedShadowSpacePixelPositionPerspective_id86(float filterRadiusInPixels, float3 pixelPositionWS, float3 meshNormalWS, float4x4 worldToShadowCascadeUV, float4x4 inverseWorldToShadowCascadeUV, out float3 adjustedPixelPositionWS, out float3 adjustedPixelPositionShadowSpace)
{
    const float4 shadowMapCoordinate = Project_id73(float4(pixelPositionWS, 1.0), worldToShadowCascadeUV);
    const float4 topRightTexelWS = Project_id73(float4(shadowMapCoordinate.xy + ShadowMapTextureTexelSize_id94.xy * filterRadiusInPixels, shadowMapCoordinate.z, 1.0), inverseWorldToShadowCascadeUV);
    const float texelDiagonalLength = distance(topRightTexelWS.xyz, pixelPositionWS);
    const float3 positionOffsetWS = meshNormalWS * texelDiagonalLength;
    adjustedPixelPositionWS = pixelPositionWS - positionOffsetWS;
    const float4 adjustedShadowMapCoordinate = Project_id73(float4(adjustedPixelPositionWS, 1.0), worldToShadowCascadeUV);
    adjustedPixelPositionShadowSpace = adjustedShadowMapCoordinate.xyz;
}
float GetFilterRadiusInPixels_id85()
{
...
float SampleTextureAndCompare_id81(float2 position, float positionDepth)
{
    return ShadowMapTexture_id92.SampleCmpLevelZero(LinearClampCompareLessEqualSampler_id127, position, positionDepth);
}
void CalculatePCFKernelParameters_id79(float2 position, out float2 base_uv, out float2 st)
{
...
float ComputeAttenuation_id109(float3 PositionWS, float3 AngleOffsetAndInvSquareRadius, float3 DirectionWS, float3 position, inout float3 lightVectorNorm)
{
    ...
    float attenuation = GetDistanceAttenuation_id107(lightVectorLength, lightInvSquareRadius);
    attenuation *= GetAngularAttenuation_id108(lightVectorNorm, lightDirection, lightAngleAndOffset.x, lightAngleAndOffset.y);
    return attenuation;
}
...
float FilterThickness_id75(float3 pixelPositionWS, float3 meshNormalWS, float2 depthRanges, float4x4 worldToShadowCascadeUV, float4x4 inverseWorldToShadowCascadeUV, bool isOrthographic)
{
    float3 adjustedPixelPositionWS;
    float3 adjustedPixelPositionShadowSpace;
    if (isOrthographic)
    {
        CalculateAdjustedShadowSpacePixelPosition_id87(GetFilterRadiusInPixels_id85(), pixelPositionWS, meshNormalWS, worldToShadowCascadeUV, inverseWorldToShadowCascadeUV, adjustedPixelPositionWS, adjustedPixelPositionShadowSpace);
    }
    else
    {
        CalculateAdjustedShadowSpacePixelPositionPerspective_id86(GetFilterRadiusInPixels_id85(), pixelPositionWS, meshNormalWS, worldToShadowCascadeUV, inverseWorldToShadowCascadeUV, adjustedPixelPositionWS, adjustedPixelPositionShadowSpace);
    }
    return SampleAndFilter_id88(adjustedPixelPositionWS, adjustedPixelPositionShadowSpace, depthRanges, inverseWorldToShadowCascadeUV, isOrthographic);
}
float FilterShadow_id74(float2 position, float positionDepth)
{
    float shadow = 0.0f;
    float2 base_uv;
    float2 st;
    CalculatePCFKernelParameters_id79(position, base_uv, st);

    {
        float3 kernel[9];
        float normalizationFactor = Get5x5FilterKernel_id82(base_uv, st, kernel);

        [unroll]
        for (int i = 0; i < 9; ++i)
        {
            shadow += kernel[i].z * SampleTextureAndCompare_id81(kernel[i].xy, positionDepth);
        }
        shadow /= normalizationFactor;
    }
    return shadow;
}
float VisibilityhSchlickGGX_id172(float alphaR, float nDotX)
{
    float k = alphaR * 0.5f;
    return nDotX / (nDotX * (1.0f - k) + k);
}
float3 FresnelSchlick_id156(float3 f0, float3 f90, float lOrVDotH)
{
    return f0 + (f90 - f0) * pow((1 - lOrVDotH), 5);
}
void ProcessLight_id110(inout PS_STREAMS streams, SpotLightDataInternal light)
{
    float3 lightVectorNorm;
    float attenuation = ComputeAttenuation_id109(light.PositionWS, light.AngleOffsetAndInvSquareRadius, light.DirectionWS, streams.PositionWS_id23.xyz, lightVectorNorm);
...
void ProcessLight_id99(inout PS_STREAMS streams, PointLightDataInternal light)
{
    float3 lightVectorNorm;
    float attenuation = ComputeAttenuation_id96(light, streams.PositionWS_id23.xyz, lightVectorNorm);
    streams.lightPositionWS_id42 = light.PositionWS;
    streams.lightColor_id44 = light.Color;
    streams.lightAttenuation_id47 = attenuation;
    streams.lightDirectionWS_id43 = lightVectorNorm;
}
float ComputeThicknessFromCascade_id78(float3 pixelPositionWS, float3 meshNormalWS, int cascadeIndex, int lightIndex, bool isOrthographic)
{
    const int arrayIndex = cascadeIndex + lightIndex * TCascadeCountBase_id154;
    return FilterThickness_id75(pixelPositionWS, meshNormalWS, DepthRanges_id159[arrayIndex], WorldToShadowCascadeUV_id156[arrayIndex], InverseWorldToShadowCascadeUV_id157[arrayIndex], isOrthographic);
}
float ComputeShadowFromCascade_id77(float3 shadowPositionWS, int cascadeIndex, int lightIndex)
{
    ...
    return FilterShadow_id74(shadowPosition.xy, shadowPosition.z);
}
float3 GetShadowPositionOffset_id76(float offsetScale, float nDotL, float3 normal)
{
...
float NormalDistributionGGX_id186(float alphaR, float nDotH)
{
    ...
    return alphaR2 / (PI_id197 * d * d);
}
float VisibilitySmithSchlickGGX_id174(float alphaR, float nDotL, float nDotV)
{
    return VisibilityhSchlickGGX_id172(alphaR, nDotL) * VisibilityhSchlickGGX_id172(alphaR, nDotV) / (nDotL * nDotV);
}
float3 FresnelSchlick_id162(float3 f0, float lOrVDotH)
{
    return FresnelSchlick_id156(f0, 1.0f, lOrVDotH);
}
float3 Compute_id264(float3 specularColor, float alphaR, float nDotV)
{
    float glossiness = 1.0f - sqrt(alphaR);
    float4 environmentLightingDFG = EnvironmentLightingDFG_LUT_id198.SampleLevel(LinearSampler_id125, float2(glossiness, nDotV), 0);
...
void PrepareDirectLightCore_id102(inout PS_STREAMS streams, int lightIndexIgnored)
{
    ...
    ProcessLight_id110(streams, spotLight);
}
...
void PrepareDirectLightCore_id91(inout PS_STREAMS streams, int lightIndexIgnored)
{
    ...
    ProcessLight_id99(streams, pointLight);
}
void PrepareLightData_id98(inout PS_STREAMS streams)
{
...
float3 ComputeShadow_id68(inout PS_STREAMS streams, float3 position, int lightIndex)
{
    ...
    shadowPosition += GetShadowPositionOffset_id76(OffsetScales_id161[lightIndex], streams.NdotL_id50, streams.normalWS_id20);
    if (cascadeIndex < TCascadeCount_id162)
    {
        shadow = ComputeShadowFromCascade_id77(shadowPosition, cascadeIndex, lightIndex);
        ...
        if (splitDist < 0.2)
        {
            ...
            {
                float nextShadow = ComputeShadowFromCascade_id77(shadowPosition, cascadeIndex + 1, lightIndex);
...
{
    return NormalDistributionGGX_id186(streams.alphaRoughness_id71, streams.NdotH_id77);
}
float Compute_id328(inout PS_STREAMS streams)
{
    return VisibilitySmithSchlickGGX_id174(streams.alphaRoughness_id71, streams.NdotL_id50, streams.NdotV_id73);
}
float3 Compute_id297(inout PS_STREAMS streams, float3 f0)
{
    return FresnelSchlick_id162(f0, streams.LdotH_id78);
}
...
void AfterLightingAndShading_id398()
{
}
float3 ComputeEnvironmentLightContribution_id449(inout PS_STREAMS streams)
{
    ...
    return Compute_id264(specularColor, streams.alphaRoughness_id71, streams.NdotV_id73) * streams.envLightSpecularColor_id49;
}
...
void PrepareEnvironmentLight_id257(inout PS_STREAMS streams)
{
...
void PrepareDirectLight_id248(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.shadowColor_id84 = ComputeShadow_id103(streams, streams.PositionWS_id23.xyz, lightIndex);
    ...
    streams.lightColorNdotL_id45 *= ComputeTextureProjection_id104(streams.PositionWS_id23.xyz, lightIndex);
    float3 reflectionVectorWS = reflect(-streams.viewWS_id69, streams.normalWS_id20);
    streams.lightSpecularColorNdotL_id46 *= ComputeSpecularTextureProjection_id105(streams.PositionWS_id23.xyz, reflectionVectorWS, lightIndex);
}
...
void PrepareDirectLight_id235(inout PS_STREAMS streams, int lightIndex)
{
    ...
    streams.NdotL_id50 = max(dot(streams.normalWS_id20, streams.lightDirectionWS_id43), 0.0001f);
    streams.shadowColor_id84 = ComputeShadow_id92(streams, streams.PositionWS_id23.xyz, lightIndex);
    streams.lightColorNdotL_id45 = streams.lightColor_id44 * streams.lightAttenuation_id47 * streams.shadowColor_id84 * streams.NdotL_id50 * streams.lightDirectAmbientOcclusion_id51;
    streams.lightSpecularColorNdotL_id46 = streams.lightColorNdotL_id45;
...
{
    PrepareLightData_id98(streams);
}
void PrepareDirectLight_id210(inout PS_STREAMS streams, int lightIndex)
{
    PrepareDirectLightCore_id67(streams, lightIndex);
    streams.NdotL_id50 = max(dot(streams.normalWS_id20, streams.lightDirectionWS_id43), 0.0001f);
    streams.shadowColor_id84 = ComputeShadow_id68(streams, streams.PositionWS_id23.xyz, lightIndex);
    streams.lightColorNdotL_id45 = streams.lightColor_id44 * streams.lightAttenuation_id47 * streams.shadowColor_id84 * streams.NdotL_id50 * streams.lightDirectAmbientOcclusion_id51;
    streams.lightSpecularColorNdotL_id46 = streams.lightColorNdotL_id45;
    streams.lightColorNdotL_id45 *= ComputeTextureProjection_id69(streams.PositionWS_id23.xyz, lightIndex);
    float3 reflectionVectorWS = reflect(-streams.viewWS_id69, streams.normalWS_id20);
    streams.lightSpecularColorNdotL_id46 *= ComputeSpecularTextureProjection_id70(streams.PositionWS_id23.xyz, reflectionVectorWS, lightIndex);
}
...
{
    float3 specularColor = streams.matSpecularVisible_id72;
    float3 fresnel = Compute_id297(streams, specularColor);
    float geometricShadowing = Compute_id328(streams);
...
{
    float3 diffuseColor = streams.matDiffuseVisible_id70;
    return diffuseColor / PI_id197 * streams.lightColorNdotL_id45 * streams.matDiffuseSpecularAlphaBlend_id66.x;
}
...
void PrepareForLightingAndShading_id395()
{
}
...
void Compute_id481(inout PS_STREAMS streams)
{
    ...
    {
        PrepareForLightingAndShading_id395();
    }
    ...
    {
        PrepareEnvironmentLight_id257(streams);
    ...
    streams.shadingColor_id74 += directLightingContribution * PI_id197 + environmentLightingContribution;
    streams.shadingColorAlpha_id75 = streams.matDiffuse_id54.a;

    {
        AfterLightingAndShading_id398();
    }
...
{
    float metalness = Compute_id262().r;
...
float3 EncodeNormal_id55(float3 n)
{
    return n * 0.5 + 0.5;
}
...
{
    return float4(EncodeNormal_id55(streams.normalWS_id20), 1);
}
...
void PSMain_id3(inout PS_STREAMS streams)
{
    ...
    streams.ColorTarget1_id3 = Compute_id48(streams);
    streams.ColorTarget2_id4 = Compute_id49(streams);
}
